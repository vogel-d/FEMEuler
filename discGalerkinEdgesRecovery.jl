function discGalerkinEdgesR!(M::Array{Float64,2},
    ...
    for e in 1:length(edgeData[1])
        ...
        for i in 1:nW
            for r in 1:sk
                w1[r]+=wval[globalNumW1[i]]*phiW[i](transformRecoveryCoord(xM1,t1,s1,transformation(m, coord1, nquadPoints[eT1][1,i], nquadPoints[eT1][2,i])));
                w2[r]+=wval[globalNumW2[i]]*phiW[i](transformRecoveryCoord(xM2,t2,s2,transformation(m, coord2, nquadPoints[eT2][1,i], nquadPoints[eT2][2,i])));
            end
        end
...
function discGalerkinEdgesR!(M::Array{Float64,2},
    ...
    nT=length(phiT);
    nF=size(phiF,2);
    nW=div(length(phiW),m.topology.size[3]);
    sk=length(quadWeights)
    ...
    for e in 1:length(edgeData[1])
        ...
        for i in 1:length(globalNumW1)
            @. w1+=wval[globalNumW1[i]]*phiWn1[(inc1-1)*nW+i];
            @. w2+=wval[globalNumW2[i]]*phiWn2[(inc2-1)*nW+i];
        end
