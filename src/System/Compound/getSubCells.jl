...
function getSubCells!(subcoord::Array{Array{Float64,2},1}, coord::Array{Float64,2}, center::Array{Float64,1}, compoundData::compoundData{:RectToTris})
    #middle=(1/6).*sum(coord,dims=2);
    fill!(center,0.0);
    for i in 1:4
        for j in 1:2
    ...
    for i in 1:4
        ...
        for j in 1:2
            subcoord[(i-1)*2+1][j,2]=0.5*(coord[j,i]+coord[j,mod(i,4)+1])
        end
        ...
        for j in 1:2
            subcoord[2*i][j,1]=0.5*(coord[j,i]+coord[j,mod(i,4)+1])
        end

        for j in 1:2
            subcoord[2*i][j,2]=coord[j,mod(i,4)+1];
        end

        for j in 1:2
            subcoord[2*i][j,3]=center[j];
        end
    end
end
