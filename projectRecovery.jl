...
function projectRecovery(degFH::degF{2,:H1div},degF::degF{2,:H1div},cval::Array{Float64,1},massMH::SuiteSparse.UMFPACK.UmfpackLU{Float64,Int64},m::mesh, kubPoints::Array{Float64,2}, kubWeights::Array{Float64,2})
    phi=@views degF.phi;
    phiH=@views degFH.phi;
    sph=size(phiH,2);
    sk=size(kubWeights);
    ...
    jphiH=initJacobi((m.geometry.dim,size(phiH,2)),sk);
    coord=Array{Float64,2}(undef,m.geometry.dim,m.meshType);
        ...
        for j in 1:sph
            for r in 1:sk[2]
                for l in 1:sk[1]
                    ...
                    gbh[globalNumH[j]]+=kubWeights[l,r]*(ddJ[l,r]^2/abs(ddJ[l,r]))*vecdot;
...
function projectRecovery(degFH::degF{2,:H1xH1},degF::degF{2,:H1xH1},cval::Array{Float64,1},massMH::SuiteSparse.UMFPACK.UmfpackLU{Float64,Int64},m::mesh, kubPoints::Array{Float64,2}, kubWeights::Array{Float64,2})
    phi=@views degF.phi;
    phiH=@views degFH.phi;
    sph=size(phiH,2);
    sk=size(kubWeights);
        ...
        for j in 1:sph
            for r in 1:sk[2]
                for l in 1:sk[1]
                    ...
                    gbh[globalNumH[j]]+=kubWeights[l,r]*abs(dJ[l,r])*vecdot;
                    #piola: abs(dJ)*ddJ=ddJ/abs(ddJ)
                end
            end
        end
