function dfft(z::OffsetArray{Complex{Float64},1}) #pfft.f90: zdfft und sdfft und scfft
...
function fftco(n::Int64) #pfft.f90 (sfftco)
   ...
   bsize=2048; bsizem=bsize-1; bsize2=bsize*2;
   ult=1; # Column index of js and ws for latest fft coefficients used.
   #data js/bsize2*0/,ws/bsize2*zero/ bsize2=bsize*2;
   js=OffsetArray(zeros(Int64,bsize,2), 0:bsizem,1:2);
   ws=OffsetArray(zeros(bsize,2), 0:bsizem,1:2);
   ln2,ln3,ln4,ln5,ln7,lnb,lnd,ln=sget2357bd(n);
   nm=n-1;nh=Int64(n/2);
   j=OffsetArray{Int64,1}(undef,0:nm);
   w=OffsetArray{Float64,1}(undef,0:nm);
   if (n <= bsize)
      if (n == js[0,ult])
      ...
      j,w=rumble(ln2,ln3,ln4,ln5,ln7,lnb,lnd,ln,n,nm,nh)
      js[0:nm,ult]=j[0:nm]; ws[0:nm,ult]=w[0:nm]
   else
      j,w=rumble(ln2,ln3,ln4,ln5,ln7,lnb,lnd,ln,n,nm,nh) # Too big to record in js and ws
   end
   return j,w,ln2,ln3,ln4,ln5,ln7,lnb,lnd;
end

function sget2357bd(nin::Int64) #pfft.f90
   ...
   return ln2,ln3,ln4,ln5,ln7,lnb,lnd,ln;
end

function snfftln(n1::Int64,m::Int64) #pfft.f90
...
function rumble(ln2::Int64,ln3::Int64,ln4::Int64,ln5::Int64,ln7::Int64,lnb::Int64,lnd::Int64,ln::Int64,n::Int64,nm::Int64,nh::Int64) #pfft.f90
   ...
   pi2on=pi2/n;
   jumble=OffsetArray{Int64,1}(undef,0:nm);
   tumble=OffsetArray{Float64,1}(undef,0:nm);
   for i in 0:nh-1
      ang=pi2on*i;
   ...
   nd=Array{Int64,1}(undef,ml);
   md=Array{Int64,1}(undef,ml)
   ...
   for i in 1:nm
      ir=i; j=0
      for l in 1:ln
         kd=Int64(floor(ir/nd[l])); ir-=kd*nd[l]; j+=kd*md[l];
      end
      jumble[i]=j
   end
   for i in 1:nm
...
function gfft(n::Int64,jumble::OffsetArray{Int64,1},w::OffsetArray{Float64,1},ln2::Int64,ln3::Int64,ln4::Int64,ln5::Int64,ln7::Int64,lnb::Int64,lnd::Int64,r::OffsetArray{Float64,1},q::OffsetArray{Float64,1}) #pfft.f90
   ...
   #  RADIX 4
   for l=1:ln4
      mb=Int64(floor(mb/4)); ma4=ma*4; mb2=mb*2
      for j=0:ma-1
         jmb=j*mb; jmb2=j*mb2
   ...
   if (ln5 > 0)
   #  RADIX 5
      nze=Int64(floor(n/5));  rze=w[nze]; qze=w[nh+nze]; rzc=1.0-rze # <- zeta
      ret=rze*rze-qze*qze;  qet=2*rze*qze; rec=1.0-ret # <- eta
      for l=1:ln5
         mb=Int64(floor(mb/5)); ma5=ma*5
         for j=0:ma-1
            jmb=j*mb;             jmb2=jmb*2
            rf1=w[jmb];           qf1=w[nh+jmb]       # f**1
            rf2=w[jmb2];          qf2=w[nh+jmb2]      # f**2
   ...
   #  RADIX 11
      ...
      for L=1:Lnb
         mb=Int64(floor(mb/11)); mab=ma*11
         for j=0:ma-1
   ...
   #  RADIX 13
      ...
      for L=1:Lnd
         mb=Int64(floor(mb/13)); mad=ma*13
         for j=0:ma-1
