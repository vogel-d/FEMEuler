function advection(p::femProblem, gamma::Float64, y::solution, Vfval::SparseVector{Float64,Int64}, Vfcomp::Symbol,
  ...
  if p.advection
    ...
    if p.taskRecovery
      ...
      vtkRecovery(p.mesh,2,2,p.degFBoundary[p.femType[:rhoTheta][3]],cR,:R1,"testRecoveryTheta")
      Sth=advectionStiffR(p.degFBoundary[fTtheta[1]],nquadPhi[fTtheta[1]],
                         ...
                         nquadPoints, p.edgeData);
      rCth=Fth\(Sth);
      vtk(p.mesh,p.degFBoundary[p.femType[:rhoTheta][1]],rCth,p.femType[:rhoTheta][1],"testThetaAdv")
    else
    ...
    if p.taskRecovery
      ...
      #vtkRecovery(p.mesh,p.degFBoundary[p.femType[:rho][3]],cR,:R1,"testRecoveryRho")
