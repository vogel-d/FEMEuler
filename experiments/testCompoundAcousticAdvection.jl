...
function testCompoundAcousticAdvection()
  ...
  m=generateHexMesh(0.0,10000.0,0.0,10000.0,40,:periodic,:periodic,meshType=4); #(east/west, top/bottom)
  ...
  function fp1(xz::Array{Float64,1})
        x=xz[1]; z=xz[2];
        return b0*sin(pi*z/H)/(1+((x-xM)/A)^2);
  end
  #function fp2(xz::Array{Float64,1})
  #  x=xz[1]; z=xz[2];
  ...
  function fp2(xz::Array{Float64,1})
    x=xz[1]; z=xz[2];
    return (z>4000.0 && z<6000.0)*1.0
    #return (x>4000.0 && x<6000.0 && z>4000.0 && z<6000.0)*1.0
  end
  #fb(x,y)=1.0;
  ...
  for i=1:nIter
    ...
    if mod(i,10)==0
      p2=deepcopy(p)
      compound_unstructured_vtk(p2, sort(collect(keys(p2.solution))), [:p, :b, :v], ["Pressure", "Buoyancy", "Velocity"], "testCompoundAcousticAdvection/"*filename)
    end
    println(Time)
  end

  #Speichern des Endzeitpunktes als vtu-Datei:
  #unstructured_vtk(p, EndTime, [:p, :b, :v], ["Pressure", "Buoyancy", "Velocity"], "testCompoundAcousticAdvection/"*filename)
  #Speichern aller berechneten Zwischenwerte als vtz-Datei:
  compound_unstructured_vtk(p, sort(collect(keys(p.solution))), [:p, :b, :v], ["Pressure", "Buoyancy", "Velocity"], "testCompoundAcousticAdvection/"*filename)
