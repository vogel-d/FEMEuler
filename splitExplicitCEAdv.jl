function splitExplicit(y0::solution,Y::Array{solution,1},FY::Array{solution,1},SthY::Array{SparseMatrixCSC{Float64,Int64},1},
                      p::femProblem, Vfcomp::Symbol, Vfval::SparseVector{Float64,Int64}, gamma::Float64,
  ...
  for i in 1:stage
    ...
    FY[i]=advection(p,gamma,Y[i],Vfval,Vfcomp,nquadPoints,nquadPhi,MrT,MrV);
    ...
    symplektischerEuler!(Y[i+1],p,fSlow,nsLoc,dtauLoc,velOld,rhoVS,rhoS,rhoThetaS);
  end
  return Y[stage+1];
end
