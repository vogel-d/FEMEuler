function symplektischerEuler!(y::solution,p::femProblem,w::solution,Sth::SparseMatrixCSC{Float64,Int64},ns::Int64,dtau::Float64,
  ...
  for i in 1:ns
    copy!(velOld,y.rhoV);
    velocity!(rhoVS,p,y.rho,y.rhoTheta,Val(p.type));
...
function velocity!(rhoVS::Array{Float64,1},p::femProblem, yRho::Array{Float64,1},yRhoTheta::Array{Float64,1},type::Val{:compressible})
  ...
  rhoVS[:]=p.massM[p.femType[:rhoV][1]]\(-(p.stiffM[:vp]*pres+Grav*(p.stiffM[:vrho]*yRho)));
  return nothing;
end
